...
import { botProcessUpdate, getUserPhotoLink } from '../telegramBot/apiCommands';
...
router.get('/getUserPhoto', async (req: Request, res: Response) => {
  const userId = req.query.userId as string;
  let photoLink = '';
  if (userId) {
    // Fetch the user's Telegram photo.
    photoLink = await getUserPhotoLink(bot, userId);
  }
